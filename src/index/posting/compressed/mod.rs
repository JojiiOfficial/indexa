...
use editor::CompressedPostingEditor;
...
use bytestore::components::number_seq::compressed::iter::OwnedCompressedNumSeqIterator;
...
pub struct CompressedPostings<B> {
    pub postings: MultiFile<B>,
}

impl<B> Creatable<B> for CompressedPostings<B>
where
    B: GrowableBackend,
{
    #[inline]
...
impl<B> Initiable<B> for CompressedPostings<B>
where
    B: Backend,
{
    #[inline]
...
impl<B> EditableIndexPosting for CompressedPostings<B>
where
    B: GrowableBackend,
{
    type Editor<'a> = CompressedPostingEditor<'a, B> where Self: 'a, B: 'a;

    #[inline]
    fn editor(&mut self) -> Self::Editor<'_> {
        CompressedPostingEditor::new(self)
    }
}
...
impl<B> IndexPosting for CompressedPostings<B>
where
    B: Backend,
{
    type PostingRetriever<'a> = OwnedCompressedNumSeqIterator<FullBackend<&'a [u8]>, u64>
        where Self: 'a;
    ...
    fn posting_retriever(
        ...
        let collection = CompressedNumberSequence::init(backend).expect("Failed to init backend");
...
mod test {
    ...
    #[test]
    fn posting() {
        let mut postings = CompressedPostings::create_mem_with_capacity(10).unwrap();
